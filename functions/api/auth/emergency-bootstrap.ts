...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  ...
  const expectedToken = (env as any).EMERGENCY_BOOTSTRAP_TOKEN?.trim();
  if (!expectedToken) {
    return badRequest("Emergency bootstrap is not enabled");
  }
  ...
  const headerToken = request.headers.get("x-emergency-bootstrap-token")?.trim();
  const token = headerToken || payload.token?.trim() || "";
  if (!token) return badRequest("Emergency bootstrap token is required");
  if (token !== expectedToken) return badRequest("Invalid emergency bootstrap token");
  ...
  try {
    ...
    return serverError(err?.message || "Failed to emergency bootstrap admin");
  }
};
