// src/components/AdminRetainerStaffMessageTraffic.tsx
import { useEffect, useMemo, useState } from "react";
import { getRetainers, type Retainer, type RetainerUser } from "../lib/data";
import {
  getRetainerStaffMessages,
  type RetainerStaffMessage,
} from "../lib/retainerStaffMessages";
...
type Thread = {
  ...
  lastAt?: string;
  lastPreview?: string;
  messages: RetainerStaffMessage[];
};

export default function AdminRetainerStaffMessageTraffic() {
  ...
  const rebuild = () => {
    const rs = getRetainers();
    setRetainers(rs);

    const next: Thread[] = [];
    for (const r of rs) {
      const users: RetainerUser[] = (r.users ?? []).filter(Boolean) as any;
      for (const u of users) {
        const msgs = getRetainerStaffMessages(r.id, u.id);
        ...
        next.push({
          ...
          retainerId: r.id,
          userId: u.id,
          retainerName: (r.companyName || (r as any).name || "Retainer") as string,
  ...
  const filtered = useMemo(() => {
    ...
    return threads.filter((t) =>
      [t.retainerName, t.userName, t.userTitle ?? ""]
        .join(" ")
        .toLowerCase()
    ...
    return (
      <div className="rounded-2xl bg-slate-900/80 border border-slate-800 p-6 text-sm text-slate-300">
        <h3 className="text-lg font-semibold text-slate-50 mb-1">
          Retainer Staff Messages
        </h3>
        <p>No internal staff messages exist yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full rounded-2xl bg-slate-900/80 border border-slate-800 p-4 md:p-5 flex flex-col gap-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-400 max-w-xl">
            Internal threads between a Retainer owner and each staff user.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="md:w-2/5 lg:w-1/3 rounded-2xl border border-slate-800 bg-slate-950/80 flex flex-col min-h-0">
          <div className="px-3 pt-3 pb-2 border-b border-slate-800 space-y-2">
            ...
            <input
              ...
              placeholder="Search by retainer or staff…"
              value={search}
            ...
            {selected ? (
              <>
                ...
                <div className="text-sm font-semibold text-slate-50 truncate">
                  {selected.retainerName} · {selected.userName}
                </div>
              </>
            ) : (
                  ...
                  >
                    <div
                      className={[
                        ...
                        isOwner
                          ? "bg-sky-500/15 text-sky-50 border-sky-500/20"
                          : "bg-slate-900/70 text-slate-50 border-slate-800",
                      ].join(" ")}
                    >
                      <div className="whitespace-pre-wrap">{m.body}</div>
                      <div className="mt-1 text-[10px] text-slate-400 text-right">
                        {m.sender === "OWNER" ? "Owner" : "Staff"} ·{" "}
      ...
      <div className="text-[11px] text-slate-500">
        Total Retainers: {retainers.length} · Threads: {threads.length}
      </div>
    </div>
  );
}
